# Generated manually: chat collection payment dots + version 0.2.21.
...
DEFAULT_APK_URL = (
    ...
    "cursor/chat-collection-payment-dots-e31c/dist/apk/voitos-debug.apk"
)
...
def bump_app_settings_rows(apps, schema_editor):
    ...
    for cfg in AppSettings.objects.all():
        changed = False
        if int(getattr(cfg, "mobile_min_version_code", 0) or 0) < 25:
            cfg.mobile_min_version_code = 25
            changed = True
        if int(getattr(cfg, "mobile_latest_version_code", 0) or 0) < 25:
            cfg.mobile_latest_version_code = 25
            changed = True
        if (getattr(cfg, "mobile_latest_version_name", None) or "").strip() != "0.2.21-kmp":
            cfg.mobile_latest_version_name = "0.2.21-kmp"
            changed = True
...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ("database", "0063_executor_role_proposal"),
    ]
    ...
    operations = [
        migrations.AlterField(
            ...
            field=models.PositiveIntegerField(
                default=25, verbose_name="Актуальный versionCode"
            ),
        ),
        migrations.AlterField(
            model_name="appsettings",
