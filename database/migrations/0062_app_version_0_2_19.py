# Generated manually: subscription gates + chat author SET_NULL + version 0.2.19.
...
def bump_app_settings_rows(apps, schema_editor):
    ...
    for cfg in AppSettings.objects.all():
        changed = False
        if int(getattr(cfg, "mobile_min_version_code", 0) or 0) < 23:
            cfg.mobile_min_version_code = 23
            changed = True
        if int(getattr(cfg, "mobile_latest_version_code", 0) or 0) < 23:
            cfg.mobile_latest_version_code = 23
            changed = True
        if (getattr(cfg, "mobile_latest_version_name", None) or "").strip() != "0.2.19-kmp":
            cfg.mobile_latest_version_name = "0.2.19-kmp"
            changed = True
...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ("database", "0061_groupchatmessage_author_set_null"),
    ]
    ...
    operations = [
        migrations.AlterField(
            ...
            field=models.PositiveIntegerField(
                default=23, verbose_name="Актуальный versionCode"
            ),
        ),
        migrations.AlterField(
            model_name="appsettings",
