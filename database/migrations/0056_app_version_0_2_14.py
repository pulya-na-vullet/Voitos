# Generated manually for subscription paywall + version 0.2.14.
...
def bump_app_settings_rows(apps, schema_editor):
    ...
    for cfg in AppSettings.objects.all():
        changed = False
        if int(getattr(cfg, "mobile_min_version_code", 0) or 0) < 18:
            cfg.mobile_min_version_code = 18
            changed = True
        if int(getattr(cfg, "mobile_latest_version_code", 0) or 0) < 18:
            cfg.mobile_latest_version_code = 18
            changed = True
        if (getattr(cfg, "mobile_latest_version_name", None) or "").strip() != "0.2.14-kmp":
            cfg.mobile_latest_version_name = "0.2.14-kmp"
            changed = True
...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ("database", "0055_app_version_0_2_13"),
    ]
    ...
    operations = [
        migrations.AlterField(
            ...
            field=models.PositiveIntegerField(
                default=18, verbose_name="Актуальный versionCode"
            ),
        ),
        migrations.AlterField(
            model_name="appsettings",
