# Generated manually: app 0.2.31 — hard update 426 + nav insets + copy.
...
def bump_app_settings_rows(apps, schema_editor):
    ...
    for cfg in AppSettings.objects.all():
        cfg.mobile_min_version_code = max(
            int(getattr(cfg, "mobile_min_version_code", 0) or 0), 35
        )
        cfg.mobile_latest_version_code = max(
            int(getattr(cfg, "mobile_latest_version_code", 0) or 0), 35
        )
        cfg.mobile_latest_version_name = "0.2.31-kmp"
...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ("database", "0074_app_version_0_2_30"),
    ]
    ...
    operations = [
        migrations.AlterField(
            ...
            field=models.PositiveIntegerField(
                default=35, verbose_name="Актуальный versionCode"
            ),
        ),
        migrations.AlterField(
            model_name="appsettings",
