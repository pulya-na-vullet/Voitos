# Generated manually for executor rating in app + version 0.2.13.
...
def bump_app_settings_rows(apps, schema_editor):
    ...
    for cfg in AppSettings.objects.all():
        changed = False
        if int(getattr(cfg, "mobile_min_version_code", 0) or 0) < 17:
            cfg.mobile_min_version_code = 17
            changed = True
        if int(getattr(cfg, "mobile_latest_version_code", 0) or 0) < 17:
            cfg.mobile_latest_version_code = 17
            changed = True
        if (getattr(cfg, "mobile_latest_version_name", None) or "").strip() != "0.2.13-kmp":
            cfg.mobile_latest_version_name = "0.2.13-kmp"
            changed = True
...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ("database", "0054_app_version_0_2_12"),
    ]
    ...
    operations = [
        migrations.AlterField(
            ...
            field=models.PositiveIntegerField(
                default=17, verbose_name="Актуальный versionCode"
            ),
        ),
        migrations.AlterField(
            model_name="appsettings",
