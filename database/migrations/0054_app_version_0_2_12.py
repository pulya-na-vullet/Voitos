# Generated manually for version display release 0.2.12.
...
def bump_app_settings_rows(apps, schema_editor):
    ...
    for cfg in AppSettings.objects.all():
        changed = False
        if int(getattr(cfg, "mobile_min_version_code", 0) or 0) < 16:
            cfg.mobile_min_version_code = 16
            changed = True
        if int(getattr(cfg, "mobile_latest_version_code", 0) or 0) < 16:
            cfg.mobile_latest_version_code = 16
            changed = True
        if (getattr(cfg, "mobile_latest_version_name", None) or "").strip() != "0.2.12-kmp":
            cfg.mobile_latest_version_name = "0.2.12-kmp"
            changed = True
...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ("database", "0053_app_version_0_2_11"),
    ]
    ...
    operations = [
        migrations.AlterField(
            ...
            field=models.PositiveIntegerField(
                default=16, verbose_name="Актуальный versionCode"
            ),
        ),
        migrations.AlterField(
            model_name="appsettings",
