# Generated manually: cabinet avatar preview + contractors by locality + 0.2.24.
...
DEFAULT_APK_URL = (
    ...
    "cursor/cabinet-avatar-contractors-locality-e31c/dist/apk/voitos-debug.apk"
)
...
def bump_app_settings_rows(apps, schema_editor):
    ...
    for cfg in AppSettings.objects.all():
        changed = False
        if int(getattr(cfg, "mobile_min_version_code", 0) or 0) < 28:
            cfg.mobile_min_version_code = 28
            changed = True
        if int(getattr(cfg, "mobile_latest_version_code", 0) or 0) < 28:
            cfg.mobile_latest_version_code = 28
            changed = True
        if (getattr(cfg, "mobile_latest_version_name", None) or "").strip() != "0.2.24-kmp":
            cfg.mobile_latest_version_name = "0.2.24-kmp"
            changed = True
...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ("database", "0066_app_version_0_2_23"),
    ]
    ...
    operations = [
        migrations.AlterField(
            ...
            field=models.PositiveIntegerField(
                default=28, verbose_name="Актуальный versionCode"
            ),
        ),
        migrations.AlterField(
            model_name="appsettings",
