# Generated manually: app 0.2.32 — WR submit non-blocking + idempotent.
...
DEFAULT_APK_URL = (
    ...
    "cursor/wr-submit-timeout-e31c/dist/apk/voitos-debug.apk"
)
...
def bump_app_settings_rows(apps, schema_editor):
    ...
    for cfg in AppSettings.objects.all():
        cfg.mobile_min_version_code = max(
            int(getattr(cfg, "mobile_min_version_code", 0) or 0), 36
        )
        cfg.mobile_latest_version_code = max(
            int(getattr(cfg, "mobile_latest_version_code", 0) or 0), 36
        )
        cfg.mobile_latest_version_name = "0.2.32-kmp"
...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ("database", "0075_app_version_0_2_31"),
    ]
    ...
    operations = [
        migrations.AlterField(
            ...
            field=models.PositiveIntegerField(
                default=36, verbose_name="Актуальный versionCode"
            ),
        ),
        migrations.AlterField(
            model_name="appsettings",
