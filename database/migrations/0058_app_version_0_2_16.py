# Generated manually: force logout when user deleted + version 0.2.16.
...
def bump_app_settings_rows(apps, schema_editor):
    ...
    for cfg in AppSettings.objects.all():
        changed = False
        if int(getattr(cfg, "mobile_min_version_code", 0) or 0) < 20:
            cfg.mobile_min_version_code = 20
            changed = True
        if int(getattr(cfg, "mobile_latest_version_code", 0) or 0) < 20:
            cfg.mobile_latest_version_code = 20
            changed = True
        if (getattr(cfg, "mobile_latest_version_name", None) or "").strip() != "0.2.16-kmp":
            cfg.mobile_latest_version_name = "0.2.16-kmp"
            changed = True
...
class Migration(migrations.Migration):
    ...
    dependencies = [
        ("database", "0057_app_version_0_2_15"),
    ]
    ...
    operations = [
        migrations.AlterField(
            ...
            field=models.PositiveIntegerField(
                default=20, verbose_name="Актуальный versionCode"
            ),
        ),
        migrations.AlterField(
            model_name="appsettings",
