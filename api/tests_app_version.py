...
class AppVersionHealthTests(TestCase):
    def setUp(self):
        ...
        cfg.mobile_latest_version_name = "0.2.6-kmp"
        cfg.save()

    def test_health_includes_version_and_update_flag(self):
        old = self.client.get("/api/v1/health?version_code=9")
        self.assertEqual(old.status_code, 200)
        data = old.json()
        self.assertTrue(data["ok"])
        self.assertEqual(data["min_app_version_code"], 10)
        self.assertTrue(data["update_required"])

        cur = self.client.get("/api/v1/health?version_code=10")
