...
def help_message(user: BotUser) -> str:
    ...
    return (
        ...
        "• Оплата подписки — фото чека; детали: «подписка»\n"
        "• Сервисные сборы (снег, двор, свет, дорога) — команда «сборы», "
        "оплата фото-чеком по предложению\n"
        "• Анкета — «регистрация» (имя, телефон, адрес)\n\n"
        "Команды\n"
        "• помощь — эта подсказка\n"
        "• подписка — статус, дни, условия и комментарии администратора\n"
        "• сборы — доступные сервисные мероприятия и шкала сбора\n"
