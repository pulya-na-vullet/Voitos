...
_FAMILY_PROMPT = (
    "По этому адресу уже есть житель.\n"
    "Вы живёте вместе / одна семья?\n"
    "Ответьте «да» или «нет»."
)
...
_STEP_PROMPTS = {
    "name": "Как вас зовут?",
    "phone": "Номер телефона (например 89625507832).",
    "address": "Адрес проживания (улица, дом, квартира).",
    "locality": "Населённый пункт (например: Куюки).",
}
...
def start_registration(user: BotUser, pending: PendingAction) -> str:
    ...
    if missing:
        return (
            ...
            "Регистрация здесь, в боте: укажите телефон — мы привяжем его к вашему Max ID.\n"
            "После этого можно войти в мобильное приложение по этому номеру "
            "(код придёт сюда же).\n\n"
