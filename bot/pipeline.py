...
class MessagePipeline:
    ...
    def handle(self, user: BotUser, text: str, *, is_voice: bool = False, voice_transcript: str = "") -> str:
        ...
        from services.mobile_auth import (
            APP_LOGIN_PENDING,
            APP_REGISTER_PENDING,
            bot_handle_app_login_phone,
            bot_handle_app_register_phone,
            bot_start_app_login,
            bot_start_app_register,
            looks_like_app_login,
            looks_like_app_register,
        )
        ...
        if pending.pending_kind == APP_REGISTER_PENDING:
            reply = bot_handle_app_register_phone(user, text)
            self._store_out(user, reply, "app_register")
            return reply
        ...
        if looks_like_app_register(text):
            reply = bot_start_app_register(user)
            self._store_out(user, reply, "app_register")
            return reply
        ...
        if pending.pending_kind == "work_request_service_survey":
            from services.work_request_client_survey import handle_service_survey_step

            reply = handle_service_survey_step(user, text, pending)
            self._store_out(user, reply, "work_request_service_survey")
            return reply
        ...
        if pending.pending_kind == "manager_survey":
            from services.manager_survey import handle_manager_survey_step

            reply = handle_manager_survey_step(user, text, pending)
            if reply is not None:
                self._store_out(user, reply, "manager_survey")
                return reply
